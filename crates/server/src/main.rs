...
async fn get_equipos(State(state): State<AppState>, _filter: Option<Query<EquipoFilter>>) -> Json<Vec<Equipo>> {
    ...
    if let Some(area) = filter.area.filter(|value| !value.trim().is_empty()) {
        query.push(if has_where { " AND " } else { " WHERE " });
        query.push("area = ");
...
async fn update_equipo(State(state): State<AppState>, Path(id): Path<i32>, Json(payload): Json<Equipo>) -> Json<Option<Equipo>> {
    ...
    let res = sqlx::query_as::<_, Equipo>(
        "UPDATE equipos SET
            ...
            observaciones = $12,
            tipo_dispositivo = $13,
            ubicacion_tecnica = $14,
            modificado_por = $15,
            modificado_por_id = $16,
...
async fn get_audit_log(State(state): State<AppState>, Path(ip): Path<String>) -> Json<Vec<AuditLog>> {
    let logs = sqlx::query_as::<_, AuditLog>("SELECT * FROM equipo_audit_log WHERE equipo_ip = $1 ORDER BY fecha DESC")
        .bind(ip)
        .fetch_all(&state.pool)
        .await
        .unwrap_or_default();
...
async fn get_all_audit_logs(State(state): State<AppState>) -> Json<Vec<AuditLog>> {
    let logs = sqlx::query_as::<_, AuditLog>("SELECT * FROM equipo_audit_log ORDER BY fecha DESC LIMIT 200")
        .fetch_all(&state.pool)
        .await
        .unwrap_or_default();
    Json(logs)
...
async fn scan_network(State(state): State<AppState>, Query(query): Query<ScanQuery>) -> Json<Vec<(String, bool)>> {
    let ips = if let Some(ip) = query.ip {
        ...
        let rows = sqlx::query("SELECT ip_address FROM equipos").fetch_all(&state.pool).await.unwrap_or_default();
